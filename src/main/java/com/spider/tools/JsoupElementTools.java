...
public class JsoupElementTools {
	...
	public static StringBuffer checkPathNoIndex(Element document,
			Element element, StringBuffer cssPath) {
	 ...
	 */
	// StringBuffer cssPath = new StringBuffer();
	public static StringBuffer checkPath(Element document, Element element,
		...
		if (element.hasAttr("id")) {
			...
			String classStr = element.attr("class").trim().replace(" ", ".");
			if (!"".equals(classStr)) {
				cssPath.insert(0, tagStr + "." + classStr + ":nth-child("
						+ index + ")");
			} else {
