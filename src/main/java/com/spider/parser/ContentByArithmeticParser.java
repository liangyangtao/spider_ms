...
public class ContentByArithmeticParser {
	...
	public void removeNoNeedElementsByCssQuery(Element contentElement) {
		String cssQuerys[] = new String[] { "script", "style", "textarea",
				"select", "noscript", "input", "ins", "li > a" };
	...
	public Element getContentBody(Element body) {
		...
		while (true) {
			String parentElementText = parentElement.text()
					.replaceAll("\\s{0,10}", "").replace(" ", "");
			String bodyElementText = bodyElement.text()
					.replaceAll("\\s{0,10}", "").replace(" ", "");
			float size = (float) parentElementText.length()
					/ bodyElementText.length();
			if (size < 0.26) {
